...
st.markdown(
    """
    <div class="hero-box">
        <div class="hero-title">Redrob AI Recruiter Dashboard</div>
        <div class="hero-subtitle">
            AI-powered candidate discovery, ranking, explainability, and recruiter decision support.
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
role_filter = st.sidebar.text_input("Filter by Role")
candidate_search = st.sidebar.text_input("Search Candidate ID")
...
if candidate_search:
    filtered_df = filtered_df[
        ...
        .str.contains(candidate_search, case=False, na=False)
    ]
...
with insight_col1:
    st.metric(
        ...
        insights["strong_fit_count"]
    )

with insight_col2:
    st.metric(
        "Good Fit Candidates",
        insights["good_fit_count"]
    )
...
with insight_col4:
    st.metric(
        "Top Candidate",
        insights["top_candidate"]
    )

st.subheader("Top Talent Picks")
...
for idx, (_, row) in enumerate(top_3.iterrows()):
    ...
    with talent_cols[idx]:
        with st.container(border=True):
            st.markdown(f"## {medals[idx]} Rank {int(row['rank'])}")
...
for _, row in filtered_df.iterrows():
    ...
    experience = "NA"
    if candidate:
        experience = candidate.get("profile", {}).get(
...
candidate_options = filtered_df["candidate_id"].tolist()
